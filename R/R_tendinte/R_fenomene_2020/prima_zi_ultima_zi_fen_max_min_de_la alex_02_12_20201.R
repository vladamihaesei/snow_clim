...
library(dplyr)
#options(warn = 2) 
tmin <- read.csv("tab/TMIN_1961_2019.csv")#temp minima 
tmin <-tmin[1:3] 
...
#t1 <- t1 %>% filter(COD == 656555)# alege  o statie 

numeorig <- names(t1)[3]
colnames(t1)[3] <- "ZAPADA"
t1 <- left_join(nebt,t1)#join nebuloz
t1 <- left_join(t1,tmin)
#t1$ZAPADA[is.na(t1$NETB) && is.na(t1$ZAPADA)] <- NA
#t1$ZAPADA[t1$NEBT >= 0 && is.na(t1$ZAPADA)] <- 0
#t1$ZAPADA[is.na(t1$NEBT)] <- NA # asta trebuie corectata
#t1$ZAPADA[t1$NEBT >= 0 && is.na(t1$ZAPADA)] <- 0 # asta e bine
#t1$ZAPADA[t1$TMIN > 5.0 && t1$ZAPADA >0] <- NA
...
#t1 <- t1 %>% filter(COD == 407500)
t1 <- t1[-c(2137439), ] # elimina data de 2012/mai24
#
t1 <- t1 %>% filter(COD != 509940) #elimina Sulina pentru strat de zapada
#t1$ZAPADA[is.na(t1$ZAPADA)] <- 0
t1 <- na.omit(t1)
summary(t1)
...
for (i in 1:length(cods)) {
  ...
  for (j in 1:(length(ani) - 1)) {
    ...
    # nu calcula daca sunt temperaturi minime >5 grade si fenomen de iarna
    if(t3$TMIN > 5 && t3$ZAPADA > 0) next
    #if (t3$TMIN > 2.0 && t3$ZAPADA > 0.0){
     # next
    #}
  ...
  di <- data.frame(nume = ws$NUME[ws$CODGE %in% cods[i]], cod = cods[i], utlima.zi.med = format(ultima.zi.med.dat, "%b%d"),
                   utlima.zi.max = format(ultima.zi.max.dat, "%Y%b%d"), utlima.zi.min = format(ultima.zi.min.dat, "%Y%b%d"),
                prima.zi.med = format(prima.zi.med.dat, "%b%d"),  prima.zi.max = format(prima.zi.max.dat, "%Y%b%d"),
                prima.zi.min = format(prima.zi.min.dat, "%Y%b%d"))
  dww <- rbind(dww,di)
...
dww
write.csv(dww,  paste0("tab_export/prima_ultima_zi_fen_",numeorig,"_1961_2019_365_zile_v10",".csv"), row.names=F)
