...
for (i in 1:length(cods)) {
  ...
  di <- data.frame(nume = ws$NUME[ws$CODGE %in% cods[i]], cod = cods[i],
                   ...
                   prima.zi.min = format(prima.zi.min.dat, "%j"),ultima.zi.med = format(ultima.zi.med.dat, "%j"),
...
dww
write.csv(dww,  paste0("tab_export/prima_ultima_zi_fen_",numeorig,"_1961_2019_zile_juliene",".csv"), row.names=F)
