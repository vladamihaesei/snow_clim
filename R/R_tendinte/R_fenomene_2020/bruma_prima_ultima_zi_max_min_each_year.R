...
for (i in 1:length(cods)) {
  ...
  for (j in 1:(length(ani) - 1)) {
    ...
    prima <- t3 %>% filter(format(DAT, "%m") >= "08") %>% 
      summarise(prima_zi = ifelse(max(ZAPADA) == 0, NA, as.character(DAT[min(which(ZAPADA > 0))])),
                prima_zi_jul_corect  = ifelse(max(ZAPADA) == 0, NA, julian[min(which(ZAPADA > 0))]))
    
    ultima <- t3 %>% filter(format(DAT, "%m") < "08") %>% 
      summarise(ultima_zi = ifelse(max(ZAPADA) == 0, NA, as.character(DAT[max(which(ZAPADA > 0))])),
                ultima_zi_jul_corect = ifelse(max(ZAPADA) == 0, NA, julian[max(which(ZAPADA > 0))]))
  ...
  # 
  di <- data.frame(nume = ws$NUME[ws$CODGE %in% cods[i]], cod = cods[i], zfi)
  dww <- rbind(dww,di)
...
write.csv(dww,  paste0("tab_export/prima_ultima_zi_fen_",numeorig,"_years",".csv"), row.names=F)
