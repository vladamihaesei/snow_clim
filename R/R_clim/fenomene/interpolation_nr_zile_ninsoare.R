...
tabs <- grep("nr_zile_ninsoare", tabs, value = T)
tabs

ind <- unique(t1.med$indicator)
for( i in 1:length(tabs)){
  
  per1 <- strsplit(tabs[i],"/|_|.csv")[[1]][15]
  print(per1) 
  t <- read.csv(tabs[i])
  ...
  writeRaster(tt_reg_prj,paste0(drive_z,"grids_export/decadal/nr_zile_ninsoare_",ind[j],"_", per1, ".tif"))
