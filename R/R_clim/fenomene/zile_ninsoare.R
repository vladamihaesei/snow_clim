...
write.csv(ninsoare.an, paste0(drive_z,"tab_export/nr_zile"))
...
ninsoare.acum <- ninsoare.an %>%left_join(ninsoare.11)%>%left_join(ninsoare.12)%>%left_join(ninsoare.01)%>%left_join(ninsoare.02)%>%left_join(ninsoare.03)%>%left_join(ninsoare.djf)
names(ninsoare.acum)[1] <- "Data"

write.csv(ninsoare.acum, paste0(drive_z,"tab_export/zile_ninsoare_1961-2020.csv"),row.names = F)
...
write.csv(ninsoare.l, paste0(drive_z,"tab_export/zile_ninsoare_total_longer.csv"))

`%notin%` <- Negate(`%in%`)

png(paste0(drive_z,"png/temporal_evolution_monthly_1961_2020.png"), width = 2400, height = 1600, res = 200)
ggplot(data = filter(ninsoare.l,indicator %notin% c("DJF","anual")), aes(x= as.numeric(Data), y = med.statii)) + 
  ...
  facet_wrap(~factor(indicator, levels = c("noiembrie","decembrie","ianuarie","februarie","martie")))
dev.off()

png(paste0(drive_z,"png/temporal_evolution_season_annual_1961_2020.png"), width = 2200, height = 1600, res = 200)
