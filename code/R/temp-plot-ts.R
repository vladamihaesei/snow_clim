...
dat_all <- fread("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/00_ORIGINAL_DATA/TRENTINO_TUM/read_HS_manual.csv") %>% 
  melt(id.vars = "Date", variable.factor = F)
setnames(dat_all, c("date", "stn_name", "hs"))
dat_all <- dat_all[!is.na(hs)]
dat_all[, date := ymd(date)]
dat_meta <- fread("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/00_ORIGINAL_DATA/TRENTINO_TUM/list_sites_HS_manual.csv")
...
for(i_provider in all_provider){
  ...
  pdf(paste0("/mnt/CEPH_PROJECTS/ALPINE_WIDE_SNOW/OVERVIEW/01_march_meeting/fig-timeseries/", i_provider, ".pdf"),
      width = 12, height = 6)
  ...
  for(i_stn_name in all_stn_name){
    ...
    dat_plot2_full %>% 
      ...
      tidyr::fill(stn_name, .direction = "downup") %>% 
